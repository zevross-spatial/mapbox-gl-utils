...
function _templateObject2() {
  var data = _taggedTemplateLiteral(["get(\"width\") + 3"]);
...
function _templateObject() {
  var data = _taggedTemplateLiteral(["2 + 2"]);
...
var utils = require('./index').default;

var mockMap = jest.fn(params => {
  var map;
  var style = {
    cursor: ''
  };
  return map = {
    ...
    removeLayer: jest.fn(layerId => {
      if (!map._layers.find(l => l.id === layerId)) {
        map._fire('error', {
          error: {
            message: "The layer '" + layerId + "'hello' does not exist in the map's style and cannot be removed."
          }
        });
      } else {
...
describe('Initialisation', () => {
  ...
  test('Can initialise the map',
  /*#__PURE__*/
  _asyncToGenerator(function* () {
    var map = yield utils.newMap(mockMapboxgl());
    expect(map).toBeDefined();
    expect(map.U).toBeDefined();
  }));
  test('addLayers() on load',
  /*#__PURE__*/
  _asyncToGenerator(function* () {
    var mgl = mockMapboxgl();
    var map = yield utils.newMap(mgl, {}, {
      addLayers: [{
        id: 'box',
        type: 'fill',
        fillColor: 'purple'
...
describe('setData()', () => {
  test('Calls setData with correct source', () => {
    ...
    var source = map.getSource.mock.results[0].value;
...
describe('hideSource(), showSource(), toggleSource()', () => {
  test('Hides layers attached to a source.', () => {
    ...
    expect(map.setLayoutProperty.mock.calls[1]).toEqual(['fill1', 'visibility', 'none']); // expect(map.setLayoutProperty).toBeCalledWith('fill1', 'visibility', 'none');
  });
  test('Shows layers attached to a source.', () => {
    ...
    expect(map.setLayoutProperty.mock.calls[0]).toEqual(['line1', 'visibility', 'visible']);
    expect(map.setLayoutProperty.mock.calls[1]).toEqual(['fill1', 'visibility', 'visible']); // expect(map.setLayoutProperty).toBeCalledWith('fill1', 'visibility', 'none');
  });
  test('Toggles layers attached to a source.', () => {
    ...
    expect(map.setLayoutProperty.mock.calls[0]).toEqual(['line1', 'visibility', 'visible']);
    expect(map.setLayoutProperty.mock.calls[1]).toEqual(['fill1', 'visibility', 'visible']);
    expect(map.setLayoutProperty.mock.calls[2]).toEqual(['line1', 'visibility', 'none']); // expect(map.setLayoutProperty).toBeCalledWith('fill1', 'visibility', 'none');
  });
});
...
describe('layerStyle()', () => {
  ...
  test('Works with id, source, type, props', () => {
    expect(map.U.layerStyle('mylayer', 'mysource', 'line', {
      lineWidth: 3
    })).toEqual(output);
  });
  test('Works with id, source, props', () => {
    expect(map.U.layerStyle('mylayer', 'mysource', {
      type: 'line',
      lineWidth: 3
    })).toEqual(output);
  });
  test('Works with id, props', () => {
    expect(map.U.layerStyle('mylayer', {
      source: 'mysource',
      type: 'line',
      lineWidth: 3
    })).toEqual(output);
...
describe('Hook functions return "remove" handlers', () => {
  test('clickOneLayer', () => {
    ...
    var remove = map.U.clickOneLayer('layer', console.log);
  ...
  test('hoverPointer', () => {
    ...
    var remove = map.U.hoverPointer('layer');
  ...
  test('hoverFeatureState', () => {
    ...
    var remove = map.U.hoverFeatureState('layer');
  ...
  test('hoverFeatureState with two sources', () => {
    ...
    var remove = map.U.hoverFeatureState('layer1', [['source1', 'sourceLayer1'], ['source2', 'sourceLayer2']]);
  ...
  test('hoverPopup', () => {
    ...
    var remove = map.U.hoverPopup('layer', f => f.properties.name);
    expect(map._handlers.mouseenter).toBeDefined();
  ...
  test('clickPopup', () => {
    ...
    var remove = map.U.clickPopup('layer', f => f.properties.name);
    expect(map._handlers.click).toBeDefined();
    remove();
    expect(map._handlers.click).not.toBeDefined();
  });
